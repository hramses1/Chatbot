...
def crear_evento_google_calendar(
    ...
    )  # Ajusta el offset de zona horaria (-5 para GMT-5)

    # Si la fecha y hora actuales son mayores o iguales al fin del evento propuesto, ajustar al siguiente día laboral
    if ahora >= fin_datetime:
        print(
            "La fecha y hora propuestas ya han pasado. Ajustando al siguiente día laboral..."
        )
        inicio_datetime = ahora.replace(
            hour=hora_apertura.hour,
            minute=hora_apertura.minute,
            second=0,
            microsecond=0,
        )
    ...
    while dias_busqueda <= max_dias_busqueda:
        ...
        if (
            is_within_working_hours(inicio_datetime, hora_apertura, hora_cierre)
            and is_within_working_hours(fin_datetime, hora_apertura, hora_cierre)
            and inicio_datetime.date() == fin_datetime.date()
        ):
            ...
            try:
                ...
                print(
                    f"Fecha y hora de inicio: {inicio_datetime.strftime('%Y-%m-%d %H:%M')}"
                )
                print(f"Fecha y hora de fin: {fin_datetime.strftime('%Y-%m-%d %H:%M')}")

                busy_times = verificar_disponibilidad(
                    service, calendar_ids, time_min, time_max
                )
        ...
        if fin_datetime.time() > hora_cierre or inicio_datetime.time() < hora_apertura:
            inicio_datetime = inicio_datetime.replace(
                hour=hora_apertura.hour,
                minute=hora_apertura.minute,
                second=0,
                microsecond=0,
            ...
            dias_busqueda += 1
            print(
                f"Cambiando al siguiente día laboral: {inicio_datetime.strftime('%Y-%m-%d')}"
            )
            continue
